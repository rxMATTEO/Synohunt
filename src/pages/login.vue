...
async function authViaProvider (provider: OAuthProviderType, extraOptions?: Credentials) {
  await signIn(provider, { callbackUrl: import.meta.env.VITE_AUTH_ORIGIN + '/dashboard', ...extraOptions });
}
</script>
    ...
    <div class="surface-card p-4 shadow-2 border-round lg:w-4">
      ...
      <form @submit.prevent="authViaProvider('credentials', { email, password })">
        <div>
          ...
          <InputText id="email1" v-model="email" required type="text" class="w-full mb-3" />

          <label for="password1" class="block text-900 font-medium mb-2">Password</label>
          <InputText id="password1" v-model="password" required type="password" class="w-full mb-3" />

          <div class="flex align-items-center justify-content-between mb-6">
            <div class="flex align-items-center">
              <Checkbox id="rememberme1" v-model="isRememberingMe" :binary="true" class="mr-2" />
              <label for="rememberme1">Remember me</label>
            </div>
            <a class="font-medium no-underline ml-2 text-blue-500 text-right cursor-pointer">Forgot password?</a>
          </div>

          <Button label="Sign In" icon="pi pi-user" type="null" class="w-full" />
        </div>
      </form>
    </div>
  </div>
